  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="bg-gradient-to-r from-cyan-500 to-blue-500 rounded-2xl p-6 text-white">
        <h2 className="text-2xl font-bold mb-2">欢迎回来，{user?.name || user?.real_name || '管理员'}！</h2>
      ...
      <motion.div variants={itemVariants} className="bg-white rounded-xl p-6 shadow-sm border border-slate-100">
        ...
        <div className="space-y-4">
          {[
            { action: '创建新订单', user: '张三', time: '5分钟前', type: 'create' },
            { action: '审核通过资料', user: '李四操作员', time: '15分钟前', type: 'approve' },
            { action: '更新订单状态', user: '王五业务员', time: '30分钟前', type: 'update' },
            { action: '添加预约', user: '赵六资料员', time: '1小时前', type: 'appointment' },
            { action: '分配订单', user: '张三', time: '2小时前', type: 'assign' },
          ].map((log, index) => (
